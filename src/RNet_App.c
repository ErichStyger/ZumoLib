...
void RNETA_SetSrcAddr(RNWK_ShortAddrType addr) {
  (void)RNWK_SetThisNodeAddr(addr);
}
...
static uint8_t HandleDataRxMessage(RAPP_MSG_Type type, uint8_t size, uint8_t *data, RNWK_ShortAddrType srcAddr, bool *handled, RPHY_PacketDesc *packet) {
  ...
  (void)packet;
  switch(type) {
    case RAPP_MSG_TYPE_BEEP:
    #if PL_CONFIG_USE_BUZZER
      *handled = TRUE;
      uint16_t freq, duration;

      freq = McuUtility_GetValue16LE(data);
      duration = McuUtility_GetValue16LE(data+2);
      Buzzer_Beep(freq, duration);
      //SHELL_ParseCommand("buzzer beep 100 200");
      #else
      *handled = TRUE;
      #endif
      break;
...
static uint8_t PrintStatus(const McuShell_StdIOType *io) {
  ...
  McuUtility_strcatNum8Hex(buf, sizeof(buf), RNETA_GetDestAddr());
#else
  McuUtility_strcatNum16Hex(buf, sizeof(buf), RNETA_GetDestAddr());
#endif
  ...
  if (McuUtility_strcmp((char*)cmd, (char*)McuShell_CMD_HELP)==0 || McuUtility_strcmp((char*)cmd, (char*)"rapp help")==0) {
    ...
    *handled = TRUE;
    if (McuUtility_ScanDecimal8uNumber(&p, &val8)==ERR_OK) {
      (void)RAPP_SendPayloadDataBlock(&val8, sizeof(val8), (uint8_t)RAPP_MSG_TYPE_DATA, RNETA_GetDestAddr(), RPHY_PACKET_FLAGS_NONE); /* only send low byte */
    } else {
    ...
    *handled = TRUE;
    if (McuUtility_ScanDecimal8uNumber(&p, &val8)==ERR_OK) {
      (void)RAPP_SendPayloadDataBlock(&val8, sizeof(val8), (uint8_t)RAPP_MSG_TYPE_DATA, RNETA_GetDestAddr(), RPHY_PACKET_FLAGS_NO_ACK); /* only send low byte */
    } else {
      McuShell_SendStr((unsigned char*)"ERR: wrong number format\r\n", io->stdErr);
      return ERR_FAILED;
    }
