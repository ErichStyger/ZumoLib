...
static uint8_t PrintStatus(const McuShell_StdIOType *io) {
  McuShell_SendStatusStr((unsigned char*)"esp2robot", (unsigned char*)"ESP-2-Robot (UART) status\r\n", io->stdOut);
  return ERR_OK;
}

static uint8_t PrintHelp(const McuShell_StdIOType *io) {
  McuShell_SendHelpStr((unsigned char*)"esp2robot", (unsigned char*)"Group of ESP to Robot (UART) commands\r\n", io->stdOut);
