...
/*!
 * \file
 * \brief Application debounce wrapper implementation.
...
static void OnDebounceEvent(McuDbnc_EventKinds event, uint32_t buttons) {
  ...
  if (buttons&BUTTONS_BIT_NAV_RIGHT) {
    button = BUTTONS_NAV_RIGHT;
  }
  if (buttons&BUTTONS_BIT_NAV_UP) {
    button = BUTTONS_NAV_UP;
  }
  if (buttons&BUTTONS_BIT_NAV_DOWN) {
    button = BUTTONS_NAV_DOWN;
  }
  if (buttons&BUTTONS_BIT_NAV_CENTER) {
    button = BUTTONS_NAV_CENTER;
  }
  if (buttons&BUTTONS_BIT_USER) {
    button = BUTTONS_USER;
  }
  ...
  switch(event) {
    ...
    #if PL_CONFIG_USE_DEBOUNCE
      Application_OnButtonEvent(button, event);
    #endif
      break;
