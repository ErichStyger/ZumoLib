 ...
 * \author (c) 2013-20226 Erich Styger, http://mcuoneclipse.com/
...
typedef enum {
  ...
  RAPP_MSG_TYPE_BEEP = 0x06,                    /* message to the robot for a buzzer beep signal: data: frequency (16bits), duration (16bits) */
  RAPP_MSG_TYPE_DRIVE_SPEED = 0x07,             /* message to the robot to change speed. Data: leftSpeed (16bits), rigthSpeed (16bits) */
  RAPP_MSG_TYPE_JOYSTICK_BTN = 0x54,            /* Joystick button message (data is one byte: 'a'/up, 'b'/right, 'c'/down, 'd'/left, ... 'F' and 'K') */
...
typedef enum {
  ...
  RAPP_MSG_TYPE_DATA_ID_SUMO_REFLECTANCE=16,/* 32bit value with 6 reflectance values (nibbles, 0-0xF) */
  RAPP_MSG_TYPE_DATA_ID_LED=17,             /* data: 0: off, 1: on */
  RAPP_MSG_TYPE_DATA_ID_BUTTON=18,          /* data: 0: user button released, 1: user button pressed */
  RAPP_MSG_TYPE_DATA_ID_NAV=19,             /* data: high 16bit: McuDbnc_EventKinds, low 16bit: BTN_Buttons_e */
  RAPP_MSG_TYPE_DATA_ID_ROBOT_MOVE=20,      /* data: Santa_RobotMoveStatus_e, 0: unknown, 1: stopped, 2: moving */
} RAPP_MSG_DataIDType;
